(function() {
  ...
  /* ================================================================
     ASILVA FORM ASSISTANT v2.0
     Dynamic form filling powered by structured profile data.
     Self-contained bookmarklet — runs on any domain.
     ================================================================ */

  if (window.__asilvaFormAssistantLoaded) {
    const existing = document.getElementById('asilva-fa-panel');
    if (existing) { existing.remove(); window.__asilvaFormAssistantLoaded = false; }
    else { return; }
  }
  window.__asilvaFormAssistantLoaded = true;
  ...
  function validateAndFormat(mapping) {
    ...
    if (type === 'number' && isNaN(Number(value))) {
      warnings.push('Expected numeric value');
    }
    if (type === 'email' && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) {
      warnings.push('Invalid email format');
    }
    if (type === 'url' && !/^https?:\/\//.test(value)) {
      warnings.push('Expected URL starting with http:// or https://');
    }
  ...
  function createPanel() {
    ...
    closeBtn.onclick = () => { panel.remove(); window.__asilvaFormAssistantLoaded = false; };
    ...
    function trapFocus(e) {
      ...
      if (e.key === 'Escape') { panel.remove(); window.__asilvaFormAssistantLoaded = false; }
    }
